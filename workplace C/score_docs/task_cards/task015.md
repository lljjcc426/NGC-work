...
- avg_changed_cell_ratio_same_shape: `0.0827`
...
1. `python "workplace C/neurogolf-2026-work/scripts/c_score_scan_artifacts.py" --tasks task015 --score-top-n 8 --full-validate`
2. Inspect best lower-size artifacts in `workplace C/score_docs/artifact_scans/` and compare memory/params split for `task015`.
3. If a lower-cost artifact validates, register it with `c_cost_diff_runner.py --task task015 --old-artifact <current> --new-artifact <candidate> --accept-if-better`.
