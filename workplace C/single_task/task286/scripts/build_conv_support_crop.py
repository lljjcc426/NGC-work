...
TASK='task286'; BASE=Path(r'E:/kagglegolf/submissions/downloaded_best/v93_7273_37_user_upload/onnx/task286.onnx'); OUT=TASK_DIR/'onnx'/'task286_conv_support_crop.onnx'
if __name__=='__main__':
 new_path=build(BASE,OUT); old=score_onnx(TASK,BASE,True); new=score_onnx(TASK,new_path,True); row={'task':TASK,'method':'conv_support_crop','old_cost':old.cost,'new_cost':new.cost,'delta_cost':new.cost-old.cost if new.cost is not None else None,'old_points':old.points,'new_points':new.points,'delta_points':new.points-old.points if new.points is not None else None,'examples_passed':new.examples_passed,'examples_checked':new.examples_checked,'local_valid':str(new.ok).lower(),'accepted':str(bool(new.ok and new.cost<old.cost)).lower(),'artifact_path':str(new_path)}; p=TASK_DIR/'reports'/'cost_diff_round2.csv'; f=p.open('w',newline='',encoding='utf-8'); w=csv.DictWriter(f,fieldnames=list(row));w.writeheader();w.writerow(row);f.close();print(row)
