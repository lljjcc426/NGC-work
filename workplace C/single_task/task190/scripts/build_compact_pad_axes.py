...
def build(source: Path, output: Path) -> Path:
    ...
    model.graph.initializer.extend([numpy_helper.from_array(np.array([0,0,20,20],dtype=np.int64),name="pad_pads_compact"),numpy_helper.from_array(np.array([2,3],dtype=np.int64),name="pad_axes_hw")])
    for node in model.graph.node:
        if node.op_type=="Pad":
            node.input[1]="pad_pads_compact"
            while len(node.input)<3: node.input.append("")
            node.input.append("pad_axes_hw")
    kept=[x for x in model.graph.initializer if x.name!="pad_pads"];del model.graph.initializer[:];model.graph.initializer.extend(kept)
